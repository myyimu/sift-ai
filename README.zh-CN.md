...
| **P0.5（部分落地）** | 离线内容身份/去重与按需 Topic Cloud；桌面信息雷达入口仍是下一步 | 用户能否快速看懂一段时间内接触的主要主题 |
| **后续阶段** | 稳定主题、关系图、Signal Ranking、历史基线与混合检索 | 能否可靠发现升温、新颖、跨来源扩散的弱信号 |

最后一行是已经写入规范的产品方向，不是当前代码的功能声明。P0.5 已部分落地离线内容身份层
和需明确确认的 Topic Cloud 最小闭环，但完整的桌面信息雷达形态仍在后续。Sift 先把最困难、
也最值得被信任的基础打牢：可靠捕获、清晰授权、本地事实源、确定性投影和可验证引用。
...
- P0.5 已部分落地：离线 UnitExtractor/Ledger 与按需 Topic Cloud，支持预算预览、明确的
  远程处理确认、缓存和来源详情（见 [RUNBOOK.md](RUNBOOK.md) §5.7）。
...
P0 范围以 [P0_DEMO_SCOPE.md](P0_DEMO_SCOPE.md) 为准；P0.5 当前实现进度见
[P0.5_IMPLEMENTATION_STATUS.md](P0.5_IMPLEMENTATION_STATUS.md)。完整 P0.5 产品形态尚未实现。
